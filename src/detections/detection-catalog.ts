...
export class DetectionCatalog {
  ...
  private rulesById = new Map<string, CatalogEntry>();
  private rulesByPlatform = new Map<string, CatalogEntry[]>();
  private rulesByLevel = new Map<string, CatalogEntry[]>();
  private rulesByTechnique = new Map<string, CatalogEntry[]>();
  private rulesByTactic = new Map<string, CatalogEntry[]>();
  private rulesByStigCategory = new Map<string, CatalogEntry[]>();

  // Hunting query tracking
  private huntingFiles = new Set<string>();
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.catalog = JSON.parse(raw) as CatalogManifest;
  ...
  private inferPlatform(filename: string): string {
    if (filename.startsWith('oci_') || filename.startsWith('cloud_guard_')) return 'oci';
    if (filename.startsWith('linux_') || filename.startsWith('suspicious_usage_')) return 'linux';
    if (filename.startsWith('win_') || filename.startsWith('windows_')) return 'windows';
    return 'oci';
  }
  ...
  getHuntingQuery(id: string): HuntingQuery | null {
    ...
    if (!this.huntingFiles.has(filename)) return null;
    ...
    for (const filename of this.huntingFiles) {
      ...
      if (query) {
        results.push({
          ...
          title: query.title,
          level: query.level,
          platform: (query.logsource?.product as Platform) || 'oci',
        });
      }
    }
    return results;
    ...
    return {
      ...
      indexSizes: {
        ...
        byPlatform: Object.fromEntries(
          [...this.rulesByPlatform.entries()].map(([k, v]) => [k, v.length])
        ),
        byLevel: Object.fromEntries(
          [...this.rulesByLevel.entries()].map(([k, v]) => [k, v.length])
        ),
  ...
  getMitreCoverage(): Record<string, unknown> {
    if (!this.catalog) return { loaded: false };
  ...
  getStigControls(): Record<string, unknown> {
    ...
    const controls: Record<string, { count: number; category: string; rules: string[] }> = {};
  ...
  private toCompact(entry: CatalogEntry): CompactRule {
    const validPlatforms: Platform[] = [...PLATFORMS];
    const platform = entry.platform || this.inferPlatform(entry.file);
    return {
  ...
  private loadRuleFile(filename: string): DetectionRule | null {
    try {
  ...
  private loadHuntingFile(filename: string): HuntingQuery | null {
    try {
      const filePath = path.join(this.rulesPath, 'hunting', filename);
      if (!fs.existsSync(filePath)) return null;
      const raw = fs.readFileSync(filePath, 'utf8');
      return JSON.parse(raw) as HuntingQuery;
    } catch {
      return null;
    }
  }
