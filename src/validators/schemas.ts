...
import { z } from 'zod';
import {
  TIME_RANGES, ANALYSIS_TYPES, EVENT_TYPES, MITRE_CATEGORIES,
  QUERY_CATEGORIES, DOC_TOPICS, LIFECYCLE_STATES, WIDGET_TYPES,
  SAVED_SEARCH_TYPES, ANALYTICS_TYPES, STAT_OPERATIONS, FIELD_OPERATIONS,
  CORRELATION_TYPES, ENTITY_TYPES, PARSER_TYPES, TILE_TYPES,
  DETECTION_LEVELS, PLATFORMS
} from '../types/index.js';
...
export const OCIDSchema = z.string()
  .regex(
    /^ocid1\.[a-z0-9]+\.(oc[0-9]+|oc1)\.[a-z0-9-]*\.[a-z0-9]+$/i,
    'Invalid OCI OCID format. Expected: ocid1.<resource>.<realm>.<region>.<id>'
  );
...
export const CompartmentIdSchema = z.string()
  .regex(
    /^ocid1\.compartment\.(oc[0-9]+|oc1)\.[a-z0-9-]*\.[a-z0-9]+$/i,
...
export const DashboardIdSchema = z.string()
  .regex(
    /^ocid1\.dashboard\.(oc[0-9]+|oc1)\.[a-z0-9-]*\.[a-z0-9]+$/i,
    'Invalid dashboard OCID format'
  )
...
export const SearchDetectionsSchema = z.object({
  ...
  stigCategory: z.enum(['CAT I', 'CAT II', 'CAT III'] as const).optional()
 ...
 */
export const ToolSchemas: Record<string, z.ZodObject<z.ZodRawShape>> = {
