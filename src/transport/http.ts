 ...
 */
function cleanSessions(config: HTTPTransportConfig): void {
  const now = Date.now();
 ...
 */
function getSession(req: IncomingMessage, config: HTTPTransportConfig): Session | null {
 ...
 * Set CORS headers
 */
function setCorsHeaders(req: IncomingMessage, res: ServerResponse, config: HTTPTransportConfig): void {
 ...
 */
function validateOrigin(req: IncomingMessage, config: HTTPTransportConfig): boolean {
  const origin = req.headers.origin;
 ...
 */
function handleOAuthMetadata(res: ServerResponse, config: HTTPTransportConfig): void {
  res.writeHead(200, { 'Content-Type': 'application/json' });
...
): Promise<{ server: HTTPServer; transport: StreamableHTTPServerTransport; config: HTTPTransportConfig }> {
  ...
  const requestHandler = async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      if (pathname === '/mcp' || pathname === '/' || pathname.startsWith('/mcp/')) {
        ...
        transport.handleRequest(req as any, res);
 ...
 */
export function getSessionStats(): { total: number; active: number; sessions: Session[] } {
  const config = getHTTPTransportConfig();
  cleanSessions(config);
