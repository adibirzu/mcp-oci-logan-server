...
test('initializeAuth prefers instance principal provider when available', async () => {
  const client = Object.create(LogAnalyticsClient.prototype) as LogAnalyticsClient;
  const clientState = client as unknown as Record<string, any>;
  ...
  const configCalls: Array<{ configurationFilePath?: string; profile?: string }> = [];

  clientState.buildInstancePrincipalsProvider = async () => instanceProvider;
  clientState.createConfigFileProvider = (
...
test('initializeAuth falls back to config provider when instance principal fails', async () => {
  ...
  const configInstances: Array<{ configurationFilePath?: string; profile?: string }> = [];
...
test('parseTimeRange converts all tool schema ranges to minutes', () => {
  ...
  const valueRegex = /'([^']+)'/g;
  const schemaRanges = new Set<string>();
  ...
  for (const range of schemaRanges) {
    const actual = (client as unknown as Record<string, any>).parseTimeRange(range);
    assert.equal(
      actual,
      expectedMinutes.get(range),
      `Unexpected minute conversion for ${range}`
    );
  }
