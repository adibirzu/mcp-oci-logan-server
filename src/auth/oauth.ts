 ...
 */
async function introspectToken(token: string, config: OAuthConfig): Promise<TokenInfo> {
 ...
 */
function validateTokenClaims(tokenInfo: TokenInfo, config: OAuthConfig): boolean {
 ...
 */
export async function verifyToken(token: string, config: OAuthConfig): Promise<TokenInfo | null> {
  try {
    // Check cache first
    ...
    if (config.tokenCacheEnabled && tokenInfo.active) {
      const cacheKey = await hashToken(token);
      const ttl = Math.min(
        config.tokenCacheTtl * 1000,
        tokenInfo.exp ? (tokenInfo.exp * 1000 - Date.now()) : config.tokenCacheTtl * 1000
      );
 ...
 */
export function sendUnauthorized(res: ServerResponse, config: OAuthConfig, message?: string): void {
