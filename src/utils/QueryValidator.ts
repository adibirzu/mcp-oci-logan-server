...
export class QueryValidator {
  ...
  async attemptFix(query: string): Promise<string | null> {
  ...
  private checkBasicSyntax(query: string, errors: string[]): void {
  ...
  private checkFieldNames(query: string, errors: string[], warnings: string[]): void {
    ...
    for (const match of fieldMatches) {
      ...
      const isKnownField = this.COMMON_FIELD_NAMES.some(known => 
        known.toLowerCase() === fieldName.toLowerCase()
      );
  ...
  private checkTimeFilters(query: string, warnings: string[]): void {
  ...
  private checkFunctions(query: string, errors: string[]): void {
  ...
  private checkPerformance(query: string, warnings: string[]): void {
  ...
  private fixFieldNames(query: string): string {
    let fixedQuery = query;
  ...
  private fixTimeFilters(query: string): string {
    let fixedQuery = query;
  ...
  private fixOperators(query: string): string {
    let fixedQuery = query;

    // Fix common operator issues
    const operatorFixes: { [key: string]: string } = {
      ' == ': ' = ',
      ' LIKE ': ' like ',
      ' CONTAINS ': ' contains ',
      ' IN ': ' in ',
  ...
  private getSimilarFieldNames(fieldName: string): string[] {
