...
import { PLATFORMS, DETECTION_LEVELS } from '../detections/types.js';
export function setupResourceHandlers(server, catalog) {
    // ============================================
    ...
    server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
        ...
        if (ruleMatch) {
            ...
            if (PLATFORMS.includes(ruleId)) {
                return {
                    contents: [{
                            uri,
                            mimeType: 'application/json',
                            text: JSON.stringify(catalog.searchRules({ platform: ruleId }), null, 2),
                        }],
                };
            }
            if (DETECTION_LEVELS.includes(ruleId)) {
                return {
                    contents: [{
                            uri,
                            mimeType: 'application/json',
                            text: JSON.stringify(catalog.searchRules({ level: ruleId }), null, 2),
                        }],
                };
            }
        ...
        if (mitreMatch) {
            ...
            return {
                contents: [{
                        uri,
                        mimeType: 'application/json',
                        text: JSON.stringify(catalog.searchRules({ mitreTechnique: techniqueId }), null, 2),
                    }],
            };
        }
