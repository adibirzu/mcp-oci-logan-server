 ...
 */
function getTroubleshootingHint(code) {
    const hints = {
...
export function handleError(error) {
    ...
    if (typeof error === 'object' && error !== null) {
        ...
        if ('statusCode' in err) {
            ...
            const message = err.message || err.serviceCode || 'OCI API error';
            switch (statusCode) {
                case 400:
        ...
        if ('code' in err && err.code === 'InvalidParameter') {
            const message = err.message || 'Invalid query syntax';
 ...
 */
export const Errors = {
    notFound: (resource, id) => new MCPError(id ? `${resource} with ID '${id}' not found` : `${resource} not found`, 'NOT_FOUND', { resource, id }),
    permissionDenied: (operation, details) => new MCPError(`Permission denied for operation: ${operation}`, 'PERMISSION_DENIED', details),
    validationFailed: (field, message) => new MCPError(`Validation failed for '${field}': ${message}`, 'VALIDATION_ERROR', { field }),
    querySyntax: (query, error) => new MCPError(`Query syntax error: ${error}`, 'QUERY_SYNTAX_ERROR', { query: query.substring(0, 100) + (query.length > 100 ? '...' : '') }),
    timeout: (operation, durationMs) => new MCPError(`Operation '${operation}' timed out${durationMs ? ` after ${durationMs}ms` : ''}`, 'TIMEOUT', { operation, durationMs }),
    ociError: (message, statusCode) => new MCPError(message, 'OCI_ERROR', statusCode ? { statusCode } : undefined, statusCode),
    internal: (message, details) => new MCPError(message, 'INTERNAL', details)
};
