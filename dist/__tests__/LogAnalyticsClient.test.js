...
test('initializeAuth uses environment credentials when config file is unavailable', async (t) => {
    ...
    const originalInitializeClient = LogAnalyticsClient.prototype.initializeClient;
    LogAnalyticsClient.prototype.initializeClient = async function () {
        // no-op for unit test to prevent external side effects
    };
    t.after(() => {
        LogAnalyticsClient.prototype.initializeClient = originalInitializeClient;
        for (const key of envKeys) {
    ...
    assert.ok(testClient.provider instanceof oci.common.SimpleAuthenticationDetailsProvider);
    assert.equal(testClient.provider.privateKey, process.env.OCI_PRIVATE_KEY_CONTENT);
    const providerRegion = (testClient.provider.authType && testClient.provider.authType._regionId) ||
